...
import { CreateTeacherNotAvailableDto } from './dto/constraints.dto';

@Injectable()
export class TeachersConstraintsService {
    constructor(
    ...
    async setMaxGapsPerDayMany(
        ...
        if (teachers.length !== data.length) {
            throw new NotFoundException(
                'Some teachers were not found or you do not have permission to update them',
            );
        }
        teachers.forEach((teacher) => {
            teacher.maxGapsPerDay =
                data.find((d) => d.id === teacher.id)?.maxGapPerDay ?? undefined;
        });
        await this.teacherRepository.save(teachers);
        return { updated: teachers.length };
    }
    ...
    async resetMaxGapsPerDayMany(userId: number, ids: number[]) {
        ...
        teachers.forEach((teacher) => {
            teacher.maxGapsPerDay = undefined;
        });
        await this.teacherRepository.save(teachers);
        return { reset: teachers.length };
    }
    ...
    async setNotAvailableConstraintsMany(
        userId: number,
        timetableId: number,
        dtos: CreateTeacherNotAvailableDto[],
    ) {
        ...
        const teacherIds = [...new Set(dtos.map((d) => d.teacherId))];

        const [days, hours, teachers] = await Promise.all([
            dayIds.length > 0
            ...
            teacherIds.length > 0
                ? this.teacherRepository.find({
                    where: { id: In(teacherIds), timetable: { User: { id: userId } } },
                })
                : Promise.resolve([]),
        ]);
        ...
        if (teachers.length !== teacherIds.length)
            throw new NotFoundException('Some teachers not found or access denied');
        ...
        const entities = dtos.map((dto) => {
            return this.notAvailableRepository.create({
                ...
                teacher: teacherMap.get(dto.teacherId),
        ...
        const existing = await this.notAvailableRepository.find({
            where: entities.map((e) => ({
                day: { id: e.day.id },
                hour: { id: e.hour.id },
                teacher: { id: e.teacher?.id },
            })),
            relations: { teacher: true, day: true, hour: true },
        });
        ...
        if (existing.length > 0) {
            const violations = existing.map((conflict) => {
                ...
                const teacherName = conflict.teacher?.name || 'Unknown';
                return `Teacher "${teacherName}" is already marked N/A on ${dayName} at ${hourName}`;
            });
