...
// NotAvailable DTOs
export class CreateTeacherNotAvailableDto {
    @ApiProperty({ example: 1 })
    @IsInt()
    @IsNotEmpty()
    dayId: number;
...
export class CreateTeacherNotAvailableManyDto {
    @ApiProperty({ example: 1 })
    @IsInt()
    @IsNotEmpty()
    timetableId: number;

    @ApiProperty({ type: [CreateTeacherNotAvailableDto] })
    @IsArray()
    @ValidateNested({ each: true })
    @Type(() => CreateTeacherNotAvailableDto)
    constraints: CreateTeacherNotAvailableDto[];
}
