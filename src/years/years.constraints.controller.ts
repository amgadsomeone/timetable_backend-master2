...
import {
    SetMaxGapsDto,
    ResetMaxGapsDto,
    CreateYearNotAvailableManyDto,
    DeleteNotAvailableConstraintsDto,
} from './dto/constraints.dto';

@ApiTags('Year Constraints')
@Controller('years/constraints')
@ApiBearerAuth('bearerAuth')
export class YearsConstraintsController {
    constructor(
        private readonly constraintsService: YearsConstraintsService,
    ) { }

    // MaxGaps endpoints
    @Post('max-gaps')
    @ApiOperation({ summary: 'Set max gaps per day for years' })
    ...
    @Delete('max-gaps')
    @ApiOperation({ summary: 'Reset max gaps per day for years' })
    @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
    resetMaxGapsPerDay(
    ...
    @ApiOperation({ summary: 'Get years with not available constraints' })
    getWithNotAvailable(
        @GetUserId() userId: number,
    ...
    @ApiOperation({ summary: 'Create not available constraints for years' })
    @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
    setNotAvailable(
        @GetUserId() userId: number,
        @Param('timetableId', ParseIntPipe) timetableId: number,
        @Body() dto: CreateYearNotAvailableManyDto,
    ) {
    ...
    @ApiOperation({ summary: 'Get not available constraints for years' })
    getNotAvailable(
        @GetUserId() userId: number,
        @Param('timetableId', ParseIntPipe) timetableId: number,
    ) {
    ...
    @ApiOperation({ summary: 'Delete not available constraints for years' })
    @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
    deleteNotAvailable(
        @GetUserId() userId: number,
        @Param('timetableId', ParseIntPipe) timetableId: number,
