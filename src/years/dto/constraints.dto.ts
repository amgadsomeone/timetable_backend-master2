...
// NotAvailable DTOs
export class CreateYearNotAvailableDto {
    @ApiProperty({ example: 1 })
    @IsInt()
    @IsNotEmpty()
    dayId: number;
...
export class CreateYearNotAvailableManyDto {
    @ApiProperty({ example: 1 })
    @IsInt()
    @IsNotEmpty()
    timetableId: number;

    @ApiProperty({ type: [CreateYearNotAvailableDto] })
    @IsArray()
    @ValidateNested({ each: true })
    @Type(() => CreateYearNotAvailableDto)
    constraints: CreateYearNotAvailableDto[];
}
